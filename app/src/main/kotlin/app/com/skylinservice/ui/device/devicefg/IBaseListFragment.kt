 ...
 * [IBaseListFragment.OnFragmentInteractionListener] interface
 * to handle interaction events.
 * Use the [IBaseListFragment.newInstance] factory method to
 * create an instance of this fragment.
 */
class IBaseListFragment : BaseFragment() {
    ...
    inner class MainAdapter(val itemClick: IOnClick, private val items: List<DeviceInfoContaner>) : RecyclerView.Adapter<MainAdapter.ViewHolder>() {
        ...
        inner class ViewHolder(view: View) : RecyclerView.ViewHolder(view) {
            ...
            fun bindForecast(item: DeviceInfoContaner, position: Int) {
                with(item) {
                    if (item.type_no.equals("101")  ||item.type_no.equals("103") )
                        itemView.device_icon.setImageResource(R.mipmap.icon_v02)
                    else
                        itemView.device_icon.setImageResource(R.mipmap.icon_station)
        ...
        if (posts?.ret?.toInt() == 200) {
            ...
            devicemanager_reclcyview.adapter = MainAdapter(object : IOnClick {
                override fun click(position: Int) {
                    ...
                    var intent = Intent(activity, DeviceDetailActivity::class.java)
